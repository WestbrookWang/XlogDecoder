...
public class XlogFileDecoder {
    ...
    private static RetData DecodeBuffer(byte[] _buffer, int _offset, int lastseq, StringBuilder _outbuffer) {
        ...
        int headerLen = 1 + 2 + 1 + 1 + 4 + crypt_key_len;
        int length = ByteBuffer.wrap(_buffer, _offset + 5, 4).order(ByteOrder.LITTLE_ENDIAN).getInt();

        byte[] tmpbuffer = new byte[length];

        int seq = (ByteBuffer.wrap(_buffer, retData.lastseq + 1, 2).order(ByteOrder.LITTLE_ENDIAN).getShort()) & 0x0FFFF;
...
//        char end_hour = (char)ByteBuffer.wrap(_buffer, _offset+headerLen-4-crypt_key_len-1, 1).order(ByteOrder.LITTLE_ENDIAN).get();
        ...
        try {
            ...
            _outbuffer.append(new String(tmpbuffer,"UTF-8"));
        } catch (Exception e) {
            e.printStackTrace();
...
//    public static byte[] teaKey;
//    public static byte[] getTeaKey(){
//        if(teaKey == null){
//            String pubkey_x = CommonUtils.bytesToHexString(byte_pubkey_x);
//            String pubkey_y = CommonUtils.bytesToHexString(byte_pubkey_y);
//            String pubkey = String.format("04%s%s", pubkey_x, pubkey_y);
//            teaKey = ECDHUtils.GetECDHKey(CommonUtils.hexStringToByteArray(pubkey), BYTE_PRIV_KEY);
//        }
//        return teaKey;
//    }
